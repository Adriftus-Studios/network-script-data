    ...
    - choose <context.args.get[1]>:
      ...
      - case assume:
        ...
        - else if <context.args.get[2]> == stay:
          ...
          - flag player dwisp.active.task:!
        - else if <context.args.get[2]> == disembark:
          - flag player dwisp.active.task:!
        - else:
...
dwisp_spawn_mob:
  ...
  script:
    ...
    - define start <player.flag[dwisp.active.location]>
    - define points <proc[define_curve1].context[<[start]>|<[target]>|5|90|0.25]>
    - define targets <player.flag[dwisp.active.location].find_players_within[100]>
    ...
    - while <player.has_flag[dwisp.active]>:
      - choose <player.flag[dwisp.active.task].if_null[default]>:
        ...
        - case far_idle:
          ...
          - else:
            - define target <player>
          - while <player.flag[dwisp.active.task].if_null[default]> == far_idle && <[target].is_spawned.if_null[false]>:
