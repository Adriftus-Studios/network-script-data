...
graves_handler:
  ...
  events:
    ...
    on player dies bukkit_priority:HIGHEST:
      ...
      - define location <player.location.find_blocks[cave_air|air].within[2].get[1]>
      - define duration <script[graves_config].data_key[grave_max_duration]>
      - wait 5t

      # % ██ [ find the lowest air block         ] ██
      - while <[location].below.material.name.advanced_matches_text[cave_air|air]>:
        - define location <[location].below>
      - while <[location].material.name.advanced_matches_text[cave_air|air]>:
