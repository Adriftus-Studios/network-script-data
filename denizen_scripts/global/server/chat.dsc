  ...
  script:
      ...
      - if <[channel]> == server:
        - if <yaml[chat_config].read[channels.<[channel]>.integrations.Discord.<bungee.server>.active].if_null[false]> && <yaml[chat_config].read[channels.<[channel]>.integrations.Discord.<bungee.server>.to-Discord].if_null[false]>:
          - bungeerun relay chat_send_message def:<list_single[<[raw_message].parse_color.strip_color>].include[<[Channel]>|<bungee.server>|<player.uuid>].include_single[<player.name.strip_color>].include_single[<[uuid]>].include[<[Message]>]>
