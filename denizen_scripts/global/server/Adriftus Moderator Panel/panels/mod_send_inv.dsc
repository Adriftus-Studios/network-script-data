...
mod_send_inv_events:
  ...
  events:
    on player right clicks item_flagged:SERVER in mod_send_inv:
      ...
      - run mod_log_action def:<player.uuid>|<player.flag[amp].get[uuid]>|0|<[origintodest]>|Send
      - run mod_message_discord_command def:<player.uuid>|send<&sp><player.flag[amp].get[name]><&sp><context.item.flag[SERVER]>
      - run mod_chat_notifier def:<player.uuid>|<player.flag[amp].get[uuid]> def.action:Send def.text:<bungee.server><&sp>to<&sp><context.item.flag[SERVER]>
      - adjust <player.flag[amp].get[player]> send_to:<context.item.flag[SERVER]>
      - inventory close
