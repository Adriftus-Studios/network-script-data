home_system_open:
  ...
  script:
    ...
    - foreach <player.flag[homes].if_null[<list>].pad_right[<player.flag[homes_unlocked]>].with[new].pad_right[5].with[locked]> as:name:
      ...
      - else:
        - inventory set slot:<[slots].get[<[loop_index]>]> o:feather[custom_model_data=800;display=<&a>Home<&co><&sp><&r><[name]>;flag=run_script:calipolis_teleport_to_home;flag=home_id:<[name]>] d:<[inventory]>
