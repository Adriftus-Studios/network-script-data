...
mod_kick_message:
  ...
  script:
    ...
    - define moderator <tern[<[moderator].is[==].to[Server]>].pass[Server].fail[<[moderator].as_player.name>]>
...
mod_notify_unban:
  ...
  script:
    - define moderator <tern[<[moderator].is[==].to[Server]>].pass[Server].fail[<[moderator].as_player.name>]>
    - define message "<&c>[!] <&b><[moderator]> has unbanned <[uuid].as_player.name>, who was <tern[<[global].exists>].pass[globally<&sp>].fail[]>banned for <[infraction]>, with the reason: <[reason]>"
    - if <bungee.list_servers.size||0> > 1:
      - bungeerun <bungee.list_servers> mod_message_staff def:<[message]>
    - else:
      - run mod_message_staff def:<[message]>
