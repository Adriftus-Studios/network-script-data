behrs_discord_toolbox:
  ...
  events:
    on discord reaction added:
        ...
        #^  - run discord_sendMessage def:<list[<context.group.name>|<context.channel.name>].include_single[<[message].separated_by[<n>]>]>
        - define url https://discordapp.com/channels/<context.group.id>/<context.channel.id>/<context.message.id>
        - define embed <[embed].url[<[url]>]>
        - define embed <[embed].description[<[message].separated_by[<n>]>]>
        - if !<context.is_direct>:
            - discord id:adriftusbot send_embed channel:<context.channel> embed:<[embed]>
        - else:
            - discord id:adriftusbot send_embed user:<context.author> embed:<[embed]>
