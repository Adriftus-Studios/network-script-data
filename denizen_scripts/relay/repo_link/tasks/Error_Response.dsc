    ...
    - if <[data.content].exists>:
      ...
      - foreach <[data.content]> key:script as:content:
        ...
        - else if <[data.server]> == test:
          - if <[data.script_data.line]> != (unknown):
            - define data.script_data.file_link https://github.com/Adriftus-Studios/test/blob/main/denizen_scripts/global/server/<[data.script_data.file].after[test/server/].replace[<&sp>].with[<&pc>20]><&ns>L<[data.script_data.line]>
          - else:
            - define data.script_data.file_link https://github.com/Adriftus-Studios/test/blob/main/denizen_scripts/global/server/<[data.script_data.file].after[test/server/].replace[<&sp>].with[<&pc>20]>
