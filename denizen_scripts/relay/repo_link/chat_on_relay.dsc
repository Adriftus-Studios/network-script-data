...
chat_send_message:
  ...
  script:
      ...
      # Ping Sanitization
      - if !<list[staff|admin|moderator|development].contains[<[game_channel]>]>:
    ...
    on discord message received for:a_bot:
      ...
      - if <yaml[discord_watcher].read[watched.<context.channel.id>]||null> != null && !<context.new_message.author.name.contains[Adriftus]>:
        ...
        - if <context.new_message.replied_to.exists>:
          ...
          - if <[reply_map].exists>:
            - define Hover "<&color[#F3FFAD]>Replied to<&co> <&r><[reply_map].get[message].after[<&chr[F802]>].replace[<&co>].with[].replace[<&sp><&sp><&sp><&sp>].with[]>"
        ...
        - if <[ReplyIcon].exists>:
          - define NameText <&chr[F802]><proc[msg_hover_ins].context[<list_single[<[Hover]>].include[<[Text]>].include[<[Insert]>]>]><&co><&sp><[ReplyIcon]>
        - else:
          - define NameText <&chr[F802]><proc[msg_hover_ins].context[<list_single[<[Hover]>].include[<[Text]>].include[<[Insert]>]>]><&co>
        ...
        - run discord_save_message def:<list[<[channel]>|<[uuid]>|<context.new_message.id>|<context.channel.id>].include[<[Message]>]>
...
discord_save_message:
  type: task
  debug: false
  definitions: channel|uuid|discord_id|discord_channel|message
  script:
    - yaml id:chat_history set <[channel]>_history:->:<map[discord_id=<[discord_id]>;uuid=<[UUID]>;discord_channel=<[discord_channel]>].with[message].as[<[message]>]>
