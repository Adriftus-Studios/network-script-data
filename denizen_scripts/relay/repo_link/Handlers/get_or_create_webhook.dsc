discord_get_or_create_webhook:
  ...
  script:
    ...
    - announce to_console <entry[webhooks].result.substring[2,<entry[webhooks].result.length.sub[1]>]>
    - yaml loadtext:<entry[webhooks].result.substring[2,<entry[webhooks].result.length.sub[1]>]> id:webhooks
