  ...
  events:
    ...
    on player clicks ender_chest in store_hub_mysteryShop_boxes_inventory:
      ...
      - if <yaml[global.player.<player.uuid>].read[economy.premium.current]> >= <context.item.nbt[price]>:
        ...
        - narrate "<&a>You have succesfully purchased: <&r><context.item.nbt[number]> <&e><list.pad_left[<context.item.nbt[stars]>].with[⭐].separated_by[]><&7><list.pad_right[<context.item.nbt[stars].sub[5].abs>].with[✩].separated_by[]> <&5>M<&d>y<&5>s<&d>t<&5>er<&d>y<&sp><&d>Boxes<&e>."
      - else:
