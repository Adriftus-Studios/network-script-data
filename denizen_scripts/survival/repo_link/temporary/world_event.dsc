...
world_event_progress_inventory_open:
  ...
  script:
    ...
    - define percentage <server.flag[world_event.progress]./[<script[world_event_config].data_key[server_amount_needed]>]||0>
    - if <[percentage]> > 1:
      - define percentage 1
    #- inventory set slot:5 o:world_event_bar_<[percentage].round_to_precision[0.2].*[5]> d:<[inventory]>
    - define "lore:|:<&e>Server<&co> <&b><server.flag[world_event.progress]||0>/<script[world_event_config].data_key[server_amount_needed]>  <&6>(<server.flag[world_event.progress]./[<script[world_event_config].data_key[server_amount_needed]>].*[100].round_to[2]||0>%)"
    - define lore:|:<list.pad_right[<[percentage].*[20].round_down||0>].with[<&a>⬛].pad_right[20].with[<&7>⬛].unseparated>
    - define lore:|:<&a>
    - define percentage <player.flag[world_event.progress]./[<script[world_event_config].data_key[personal_amount_needed]>]||0>
    - if <[percentage]> > 1:
      - define percentage 1
