slime_mob_handler:
  ...
  events:
    on player damaged by slime:
      ...
      - if <[item].repairable>:
        - define durability_damage <context.damager.mythicmob.level.sub.[6].mul[<context.damager.size.div[2].round_up>]>
        - if <[item].durability.add[<[durability_damage]>]> >= <[item].max_durability>:
          - playeffect effect:ITEM_CRACK at:<player.location.above[0.5].forward[0.4]> special_data:<[item]> offset:0.2 quantity:15
          - take slot:<[slot]>
        - else:
          - inventory adjust slot:<[slot]> durability:<[item].durability.add[<[durability_damage]>]>
