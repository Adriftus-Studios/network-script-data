...
# TODO Make icon for when chunks are not claimed
...
claiming_initialize_group:
  ...
  members:
    ...
    everyone:
      ...
      kill-animals: false
  settings:
    display_name: <[name].replace[_].with[<&sp>]>
    color: white
    darken_sky: false
    create_fog: false
    auto-plant: false
  ...
  - yaml id:claims set groups.<player.uuid>~<[name]>.members.<player.uuid>:<script.parsed_key[members.player]>
  - yaml id:claims set groups.<player.uuid>~<[name]>.members.everyone:<script.parsed_key[members.everyone]>
...
claiming_protection_events:
  ...
  events:
    ...
    - else if !<yaml[claims].read[groups.<[group]>.members.<player.uuid>.break]||false> && !<yaml[claims].read[groups.<[group]>.members.everyone.break]>:
      ...
      - determine cancelled
    on player clicks *door|*_button|lever|chest|ender_chest|*_gate|crafting_table|anvil|furnace|brewing_stand|enchanting_table|*_bed priority:100:
    ...
    - define group2 <yaml[claims].read[<context.destination.world>.<context.destination.chunk.x>.<context.destination.chunk.z>]||null>
    - if <[group1]> == <[group2]>:
      - stop
    - if <[group2]> != null:
      - determine cancelled
    ...
    - if <context.location.chunk> == <context.source_location.chunk>:
      - stop
    - define group1 <yaml[claims].read[<context.location.world>.<context.location.chunk.x>.<context.location.chunk.z>]||null>
    - define group2 <yaml[claims].read[<context.source_location.world>.<context.source_location.chunk.x>.<context.source_location.chunk.z>]||null>
    - if <[group1]> == <[group2]>:
      - stop
    - if <[group1]> != null:
      - determine cancelled
...
claiming_system_bossBar_Start:
  ...
  - if <[update]||null> == null:
    - title title:<[group_color]><yaml[claims].read[groups.<[group]>.settings.display_name]> subtitle:<[group].before[~].as_player.name><[apostrophe]><&sp>Claim
...
claiming_system_bossBar_OuterRealms:
  ...
  - if <server.has_flag[outerRealms_bossbar_flags]>:
    - bossbar <player.uuid>.in_claim "title:<&2>Outer Realms" color:green options:<server.flag[outerRealms_bossbar_flags]>
  - else:
...
claiming_system_bossBar_SavageLands:
  ...
  script:
      ...
      - if <server.has_flag[SavageLands_bossbar_flags]>:
        - bossbar <player.uuid>.in_claim "title:<&4>Savage Lands" color:red options:<server.flag[SavageLands_bossbar_flags]>
      - else:
...
claim_system_apply_fly_in_all_claims:
  ...
  definitions: group
  script:
  - define targets <server.notables[cuboids].filter[note_name.starts_with[claim.<[group]>]].parse[players].combine>
  - foreach <[targets]>:
...
claim_system_remove_fly_target:
  ...
  script:
  - if !<server.notables[cuboids].filter[note_name.starts_with[claim.<[group]>]].filter[players.contains[<player>]].is_empty>:
    - if <player.gamemode> == SURVIVAL:
      - adjust <player> can_fly:false
...
claiming_settings_update:
  ...
  - ratelimit <[group]> 10t
  - define update true
  - foreach <server.online_players.filter[location.cuboids.parse[note_name].filter[starts_with[claim.<[group]>]].is_empty.not]> as:target:
