    ...
    - if <context.item.has_nbt[action]>:
      - choose <context.item.nbt[action]>:
        ...
        - case location:
          - determine passively cancelled
          - playsound <player> sound:UI_BUTTON_CLICK
          - if <list[VOID|SUFFOCATION].contains[<player.last_damage.cause>]>:
            - narrate "<&c>Your death location is unsafe. You don't want to see the horrors there."
            - inventory close
            - stop
        ...
        - case confirm:
          - determine passively cancelled
          - if <list[VOID|SUFFOCATION].contains[<player.last_damage.cause>]>:
            - narrate "<&c>Your death location is unsafe. I don't know if you wanna go back there."
            - inventory close
            - stop
          - else if !<player.has_flag[player_death_location]>:
            - narrate "<&c>You do not have a death location for me to send you to."
            - playsound <player> sound:UI_BUTTON_CLICK
            - inventory close
          - else:
          ## TODO Add price to the teleport command.
            - narrate "<&a>Thank you for your tribute."
            - inventory close
