claiming_upgrades_command:
  ...
  script:
    - inventory open d:claiming_protection_group_upgrades player:<server.match_player[<context.args.get[1]>]>
...
claiming_protection_group_upgrades:
  ...
  procedural items:
    - foreach <script[claiming_system_upgrade_data].list_keys[upgrades].exclude[claim_limit]> as:upgrade:
      ...
      - define cost <script[claiming_system_upgrade_data].yaml_key[upgrades.<[upgrade]>.cost]>
      - define material <script[claiming_system_upgrade_data].yaml_key[upgrades.<[upgrade]>.material]>
    ...
    - define cost <script[claiming_system_upgrade_data].yaml_key[upgrades.claim_limit.cost]>
  ...
  events:
    on player clicks item in claiming_protection_group_upgrades:
      ...
      - if <context.item.has_nbt[upgrade]>:
        - define cost <script[claiming_system_upgrade_data].yaml_key[upgrades.<context.item.nbt[upgrade]>.cost]>
        - if <player.money> < <[cost]>:
          - narrate "<&c>You don't have the cash for that."
          - stop
        - take money quantity:<[cost]>
        - give <context.item.with[lore=<context.item.lore.get[3].to[<context.item.lore.size.-[1]>]>]>
    ...
    on player right clicks with:claiming_group_upgrade_item:
      ...
      - if <context.item.nbt[upgrade]> == claim_limit:
        ...
        - stop
      - define group <player.location.cuboids.filter[notable_name.starts_with[claim]].get[1].notable_name.after[.].before[/]||null>
      - if <[group]> != null:
